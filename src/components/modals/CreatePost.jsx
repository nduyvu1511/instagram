...
const CreatePost = () => {
  ...
  const handleCreatePost = () => {
    ...
    dispatch(
      fetchCreatePost({
        ...
        images: [`http://localhost:8080/${imageUrl}`],
  ...
  const handleSendImage = () => {
    ...
    data.append("imageName", imageFile)
    axios
      .post("http://localhost:8080/upload/image", data)
      .then((res) => {
  ...
  return (
    <>
      ...
      {/* Post Image */}
      <section
        className={`modal animate-none ${isSelectedImage ? "flex" : "hidden"}`}
      >
        <div
            ...
            <div className="w-full md:w-2/5 border-l mb-10 md:mb-0 md:h-full px-4 relative">
              <div className="my-3 flex items-center">
                <img
                  ...
                  alt=""
                />
                <p className="ml-3 font-medium text-sm">{loggedUser.name}</p>
              </div>
      ...
      <section className={`modal z-2000 ${isCancelCreatePost ? "" : "hidden"}`}>
